   ...
   stage ('liquibase commands'){
      steps {
        sh '''
          ...
          echo "------------------------------------"
          liquibase --url=${Postgres_RDS_URL} --username=${Postgres_RDS_USERNAME} --password=${Postgres_RDS_PASSWORD} --referenceUrl=$Postgres_RDS_DIFF_URL --referenceUsername=${Postgres_RDS_DIFF_USERNAME} --referencePassword=${Postgres_RDS_DIFF_PASSWORD} --contexts=$ENVIRONMENT_STEP diff
          echo "---------------------------------------------"
